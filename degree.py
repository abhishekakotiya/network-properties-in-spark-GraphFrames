...
def simple(g):
	...
	flipped_df = g.edges.rdd.map(lambda e: (e.dst, e.src)).collect()
	flipped_df = sqlContext.createDataFrame(sc.parallelize(flipped_df), ['src', 'dst'])
	...
	flipped_df = g.edges.unionAll(flipped_df).distinct()
	flipped_df = flipped_df.filter('src != dst')
	
	return GraphFrame(g.vertices, flipped_df)
...
def degreedist(g):
	# Generate a DF with degree,count
    	deg_count = g.inDegrees
    	deg_count = deg_count.selectExpr("id as id", "inDegree as degree").groupBy('degree').count()
...
def readFile(filename, large, sqlContext=sqlContext):
	...
	g_df = sqlContext.createDataFrame(lines.map(lambda part: part.split(delim)), ['src', 'dst'])
	...
	# graphframe vertices
	vertices = g_df.selectExpr("src as id").unionAll(g_df.selectExpr("dst as id")).distinct()

	# Create graphframe g from the vertices and edges.
	g = GraphFrame(vertices, g_df)
